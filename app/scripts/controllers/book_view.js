  ...
  .controller('BookViewCtrl', function ($scope, $http, $window, $location, $interval, authorization,
    ...
    $scope.showConfirmBuyElBook = function(book) {
      ...
      var confirm = $mdDialog.confirm()
        ...
        .textContent('Натискаючи кнопку «оплатити», я безумовно погоджуюсь з тим, що:\n' +
          'Придбана мною електронна книга (надалі – книга) після завантаження використовуватиметься мною тільки в особистих цілях шляхом копіювання, перегляду чи інших дій, лише на моєму обладнанні.\n' +
          ...
          'Я не маю права переробляти, поширювати, публічно показувати, публікувати в глобальній мережі Інтернет, імпортувати, здавати в прокат, публічно виконувати, доводити до загального відома книгу, розміщувати посилання на будь-яких ресурсах, щоб третя особа мала доступ до книги. \n' +
    ...
    $http.get(config.url() + '/api/books/images?code=' + code)
      .success(function (response) {
        ...
        $scope.bookInfoStyle={position: 'relative', margin: 'auto'};

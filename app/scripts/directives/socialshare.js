/**
 * Created by sergey on 02.03.17.
 */
'use strict';

angular.module('angularApp').factory('socialFB', function($window, $rootScope) {
    return {
      init: function(fbId) {
        if (fbId) {
          this.fbId = fbId;
          $window.fbAsyncInit = function() {
            ...
            if (FB !== undefined){
              $rootScope.$broadcast('FB_has_loaded')
            }
          };
          (function(d) {
            var js,
  ...
  return {
    ...
    transclude: true,
    template: '<i class="fa fa-facebook" style="font-size: large">&nbsp;&nbsp;' +
    '            <span ng-show="shares > 0" class="brand-color">{{shares}}</span>' +
    '          </i>',
    link: function(scope, element, attr) {
      scope.$on('FB_has_loaded', function() {
        if (attr.url, attr.code) {
          FB.api(
            '/http://kmbooks.com.ua/book',
            'GET',
            {"code":"709684"},
            function(res) {
              var count = (res && res.share) ? res.share.share_count.toString() : 0;
              scope.shares = count;
            }
          );
        }
...
}]).directive('bkFacebookFeedShare', ['$http', function($http) {
  return {
    scope: {
      callback: '=',
      shares: '='
...
}]).directive('bkTwitter', ['$timeout', function($timeout) {
  return {
    link: function(scope, element, attr) {
...
}]).directive('bkLinkedin', ['$timeout', '$http', '$window', function($timeout, $http, $window) {
  return {
    scope: {
      shares: '='
    },
